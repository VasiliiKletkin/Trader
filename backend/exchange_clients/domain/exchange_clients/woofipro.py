...
from exchanges.domain import Candle, Timeframe, TradingPair, WooFiProExchange
...
class WooFiProExchangeClient(AbstractExchangeClient):
    """Клиент для WOOFI PRO."""

    def __init__(
        self,
        exchange: WooFiProExchange,
        api_key: str = "API_KEY",
    ...
    async def __aenter__(self) -> "WooFiProExchangeClient":
...
if __name__ == "__main__":
    ...
    async def main():
        exchange = WooFiProExchange(name="WooFiPro")
        client = WooFiProExchangeClient(exchange=exchange, demo=False)
        tp_btc = TradingPair(
            name="BTC/USDT",
