...
class ArbitrageTraderDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        context_data = super().get_context_data(**kwargs)
        trader: Trader = self.get_object()
        orders: TraderOrder = trader.arbitrage_orders.select_related("order").order_by(
            "-order__timestamp"
        )
        positions: TraderPosition = trader.arbitrage_positions.order_by("-opened_at")

        context_data["orders"] = [order.order for order in orders]
        context_data["positions"] = positions
        context_data["dash_context"] = {
            "arbitrage-trader-id": {"data": trader.pk},
        }
        return context_data
