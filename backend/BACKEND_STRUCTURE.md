...
- `Trader` - Основная модель трейдера с конфигурацией стратегии, риск-менеджера, баланса
  ...
  - Свойство `candle_provider.first_source` - получает первичный источник свечей
...
- `TraderSignal` - Торговые сигналы (BUY/SELL/WAIT)
  - Хранит `first_candle` и `second_candle` для синтетических провайдеров
...
- `CandleProvider` - Провайдер свечей с настройками агрегации
  ...
  - `first_source` - FK к `CandleSource` (обязательно)
...
- `ProviderCandle` - расширяет `Candle`
  - `first_candle: ExchangeCandle` - первичная свеча
...
- Domain-объект с полями `first_candle` и `second_candle`
...
1. **Domain-слой** работает с `ProviderCandle` (содержит `first_candle` и `second_candle`)
   - `PlainCandleProvider.get_candle()` → `ProviderCandle` с только `first_candle`
...
2. **Метод `sync_signals()`** сохраняет новые сигналы в БД:
   ...
   - Сохраняет `first_candle` и `second_candle` как FK
3. **Метод `load()`** восстанавливает domain-объекты из БД:
   - Загружает `TraderSignal.first_candle` и `TraderSignal.second_candle`
